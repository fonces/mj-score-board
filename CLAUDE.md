...
Vue 3 製の SPA／PWA（`<script setup lang="ts">` ベース）で、麻雀のスコアを記録する。ルートは 1 つだけで、`App.vue` が `ScoreTable.vue` をマウントし、ゲーム状態はすべて `ScoreTable.vue` が保持する。

すべてのコンポーネントは `<script setup>` で書く。props はランタイム宣言（`defineProps({ foo: { type: ..., required: true } })`）に `as PropType<T>` を付けて型と validator を両立させる。`tsconfig.json` は `strict: true`（全 strict フラグ有効）で運用。`.vue` の SFC 用 shim は `src/shims-vue.d.ts`。エントリは `src/main.ts`（`vue.config.js` の `entry` で指定）。

Vue は 3.2 系のため `defineOptions` は使えない。`inheritAttrs: false` を設定するコンポーネント（`Button`/`Switch`/`TextInput`）と enum 等を export するコンポーネント（`Animate.vue` の `ANIMATION` / `AnimationName`）は、`<script setup>` の前に通常の `<script lang="ts">` ブロックを併用して対応する。
...
- ESLint 設定は `package.json` にインラインで記述（`plugin:vue/vue3-essential` + `eslint:recommended` + `@vue/typescript`）。`vue/no-reserved-component-names` と `vue/multi-word-component-names` は `Button`/`Label`/`Switch`/`Dialog` 等の単語1つの命名を許容するために off にしている。
